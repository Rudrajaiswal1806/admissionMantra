   ...
   const { threadname} = route.params;
    const renderer = ({ item, index }) => {
        if (item.id!=null) {
            return (
                <View style={{
                    marginTop: 10, width: 150, height: 150, backgroundColor: "#00a2e8", borderRadius: 20, margin: 15, marginBottom: 5,
                    shadowColor: "#000",
                    shadowOffset: {
                        width: 0,
                        height: 3,
                    ...
                    <Text style={{ textAlign: 'center', color: 'white', fontWeight: '800', fontSize: 22, padding:15 }}>Test Paper</Text>
                
    
    
                    <View style={{ borderColor: '#fff', borderWidth: 1, margin: 15 }}>
                        <TouchableOpacity onPress={() => navigation.navigate('Pdf', { thread: item.id, })}>
                            <Text style={{ textAlign: 'center', color: '#fff', fontWeight: '400', fontSize: 24, paddingTop:10, paddingBottom:10 }}>{item.name}</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            )
