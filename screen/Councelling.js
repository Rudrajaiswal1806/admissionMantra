...
const Councelling = ({ navigation, route }) => {
        ...
        return (
            <View style={{ borderColor: '#fff', borderWidth: 1, margin: 15 }}>
                <TouchableOpacity onPress={() => navigation.navigate('Paper', { thread: item.tno, threadname: item.tname })}>
                    <Text style={{ textAlign: 'center', color: 'white', fontWeight: '300',padding:10, fontSize: 24 }}>{item.tname}</Text>
                </TouchableOpacity>
            </View>
        )
    }
    ...
    <Text style={styles.usrTxt}>Hello Learner!{partdata.username}</Text>
                </View>
